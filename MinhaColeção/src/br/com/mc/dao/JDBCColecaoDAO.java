...
public class JDBCColecaoDAO implements ColecaoDAO {
    ...
    public void inserir(int idMidia) throws Exception {
        ...
        try {
            ...
            String sql = "INSERT INTO colecao (midia_idmidia) VALUES (?)";
    ...
    public void remover(int idMidia) throws Exception {
        ...
        try {
            con = ConnectionFactory.getConnection();
            String sql = "DELETE from colecao WHERE midia_idmidia = ?";
            PreparedStatement st = con.prepareStatement(sql);
            st.setInt(1, idMidia);
            st.executeUpdate();
            st.close();
    ...
    public List<Midia> listar() throws Exception {
        ...
        try {
            ...
            String sql = "SELECT * FROM midia m, colecao c WHERE c.midia_idmidia = m.idmidia";
            PreparedStatement st = con.prepareStatement(sql);
    ...
    public boolean estarRelacionado(int idMidia) {
        ...
        try {
            con = ConnectionFactory.getConnection();
            String sql = "SELECT m.idmidia FROM midia m, colecao c WHERE m.idmidia = c.midia_idmidia AND c.midia_idmidia =?";
            PreparedStatement st = con.prepareStatement(sql);
            st.setInt(1, idMidia);
            ResultSet rs = st.executeQuery();
    ...
    public List<Midia> naoEstaRelacionado() {
        try {
            con = ConnectionFactory.getConnection();
            List<Midia> listaMidia = new ArrayList<Midia>();
            String sql = "SELECT * FROM midia m WHERE m.idmidia not in (SELECT c.midia_idmidia FROM colecao c) ORDER BY m.tituloEmPortugues";
            PreparedStatement st = con.prepareStatement(sql);
